	...
	if (Dlight.intensity > 1.0E-4)
	{
		...
		float Mix = (aolTex.x + aolTex.y + aolTex.z) / 3.0f;
                ...
                vec3 PointdiffuseColor = diffuse * light.diffuse *
					vec3(mix(texture(material.overlayMap,OLTexCoord),texture(material.diffuseMap,TexCoord), Mix));	
                vec3 PointspecularColor = specular * (1.0f-light.specular) * 7.0f * 
						vec3(texture(material.specularMap,TexCoord)) * 
...
vec3 PointLight(vec3 Position,vec3 norm,Material material,Light light,PointLightVars Plight,float time,vec3 cPos)
{
        //Properties
        ...
        float offset = abs(sin(time * 0.0016));
        vec2 OLTexCoord = vec2(TexCoord.x - offset,TexCoord.y);
        float Mix = vec3(texture(material.alphaoverlayMap,OLTexCoord)).x;
...
void main()
{
	...
	output += DirectionalLight(Position,norm,material,light,Dlight,Time,CameraPos);

	//Point Lights
	for (int i = 0; i < NR_POINT_LIGHTS;++i)
	{
		if ((Plight[i].constant + Plight[i].linear + Plight[i].quadratic) > 1.0E-5)
			output += PointLight(Position,norm,material,light,Plight[i],Time,CameraPos);
	}
	//Spot lights
